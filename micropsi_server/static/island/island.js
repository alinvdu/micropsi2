...
function set_world_data(data) {
  ...
  for (var key in objects) {
    if (!(key in data.objects)) {
      ...
      if (
        data.objects[key].position &&
        data.objects[key].position.length == 2
      ) {
        if (!(path && path.objectMoved && path.name == key)) {
          objects[key].x = data.objects[key].position[0];
          objects[key].y = data.objects[key].position[1];
          objects[key].representation.position = new Point(
            objects[key].x,
            objects[key].y
          );
        }
        if (data.objects[key].orientation) {
          objects[key].representation.rotate(
            data.objects[key].orientation - objects[key].orientation
          );
        }
    ...
    if (data.objects[key].position && data.objects[key].position.length == 2) {
      if (key in data.agents) {
        addAgent(
          new WorldObject(
            key,
            data.objects[key].position[0],
            data.objects[key].position[1],
            data.objects[key].orientation,
            data.objects[key].name,
            data.objects[key].type
          )
        );
...
function updateSceneViewer() {
  ...
  for (var key in scenes) {
    selector_html +=
      '<option value="' + key + '">' + objects[key].name + "</option>";
  }
  if (selector_html != "") {
  ...
  if (scene) {
    ...
    for (var row in scene.shape_grid) {
      for (var col in scene.shape_grid[row]) {
        var classnames = [];
        if (
          scene.fovea_x + grid_factor.x / 2 == col &&
...
function loadWorldInfo() {
  api.call(
    ...
    (success = function (data) {
      ...
      if ("assets" in data) {
        var iconhtml =
          '<img src="/static/island/unknownbox.png" id="icon_default_object" /><img src="/static/island/Micropsi.png" id="icon_default_agent" />';
        for (var key in data.assets.icons) {
          iconhtml +=
            '<img src="/static/' +
            data.assets.icons[key] +
            '" id="icon_' +
            key +
            '" /> ';
        }
        ...
        canvas.css(
          "background",
          'url("/static/' + data.assets.background + '") no-repeat top left'
        );
      }
    }),
    (error = function (data) {
...
function getLegend(worldobject) {
  ...
  var height =
    viewProperties.fontSize * viewProperties.zoomFactor +
    2 * viewProperties.padding;
  var point = new Point(
  ...
  content +=
    " (" + parseInt(worldobject.x) + "/" + parseInt(worldobject.y) + ")";
  ...
  if (
    point.x + text.bounds.width + 2 * viewProperties.padding >
    view.viewSize.width
  ) {
    point = new Point(
  ...
  var container = new Path.Rectangle(
    new Point(
      point.x - viewProperties.padding,
      point.y + viewProperties.padding
    ),
    new Size(text.bounds.width + 2 * viewProperties.padding, -height)
  );
...
$("body").mousedown(function (event) {
  if (
    addObjectMode &&
    event.target != canvas[0] &&
    event.target != $("#set_worldobject_sprinkle_mode")[0]
  ) {
    unsetAddObjectMode();
  }
});
  ...
  if (movePath) {
    ...
    if (label) {
      ...
      label.position = new Point(
        ...
        Math.max(
          height,
          obj.bounds.y + viewProperties.label.y * viewProperties.zoomFactor
        )
      );
    }
    if (selectionBorder) {
...
function selectObject(worldobject) {
  ...
  var bounds = worldobject.representation.bounds;
  selectionBorder = new Path.Rectangle(
    worldobject.x - bounds.width / 2,
    worldobject.y - bounds.height / 2,
    bounds.width,
    bounds.height
  );
...
function scrollToObject(obj) {
  ...
  if (bounds.y <= parent.scrollTop()) parent.scrollTop(bounds.y - 50);
    ...
    parent.innerHeight() + parent.scrollTop()
  )
    parent.scrollTop(bounds.y - parent.innerHeight() + bounds.height + 50);
  if (bounds.x <= parent.scrollLeft()) parent.scrollLeft(bounds.x - 50);
  else if (bounds.x + bounds.width >= parent.innerWidth() + parent.scrollLeft())
    parent.scrollLeft(bounds.x - parent.innerWidth() + bounds.width + 50);
}

// --------------------------- controls -------------------------------------------------------- //

function initializeControls() {
  $('.editor_field form .controls button[type="reset"]').on(
    "click",
    showDefaultForm
  );

  $("#available_worldobjects").html(
    "<option>" + available_object_types.join("</option><option>") + "</option>"
  );
...
function showObjectForm(worldobject) {
  ...
  for (var key in worldobject.parameters) {
    param_html +=
      '<tr><td><input type="text" class="param_name inplace" name="param_name" value="' +
      key +
      '" /></td><td><input type="text" class="inplace" name="param_' +
      key +
      '" val="' +
      worldobject.params[key] +
      '" /></td></tr>';
  }
...
function createWorldObject(type, pos) {
  api.call(
    "add_worldobject",
    { world_uid: currentWorld, type: type, position: [pos.x, pos.y] },
    function (result) {
...
function deleteWorldObject(worldobject) {
  ...
  api.call(
    "delete_worldobject",
    { world_uid: currentWorld, object_uid: worldobject.uid },
    function () {
